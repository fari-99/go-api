...
func (controller *TwoFactorAuthController) CreateNewAuth(ctx *gin.Context) {
	...
	if oldTwoAuthModel != nil {
		...
		if err != nil {
			configs.NewResponse(ctx, http.StatusBadRequest, fmt.Sprintf("failed to update old 2FA model status, err := %s", err.Error()))
			return
		}
	}
	...
	secret := helpers.GenerateRandString(10, "alphanum")
	...
	if err != nil {
		configs.NewResponse(ctx, http.StatusBadRequest, fmt.Sprintf("error encrypt secret key, err := %s", err.Error()))
		return
	}
...
func (controller *TwoFactorAuthController) ValidateAuth(ctx *gin.Context) {
	...
	configs.NewResponse(ctx, http.StatusOK, fmt.Sprintf("success to authenticate"))
	return
}
...
func (controller *TwoFactorAuthController) GenerateRecoveryCode(ctx *gin.Context) {
	...
	for i := 0; i < 10; i++ {
		model := models.TwoAuthRecoveries{
			...
			Code:   helpers.GenerateRandString(8, "number"),
			Status: constant.StatusActive,
		}
		...
		if err != nil {
			tx.Rollback()
			configs.NewResponse(ctx, http.StatusBadRequest, fmt.Sprintf("error create code, please try again later, err := %s", err.Error()))
			return
		}
	...
	tx.Commit()
	configs.NewResponse(ctx, http.StatusUnauthorized, code)
	return
}
